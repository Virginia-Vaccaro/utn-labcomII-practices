...
form.addEventListener('submit', (e) => {
    ...
    if (cantidadKwh.value <= 0 || cantidadKwh.value == null) {
        mensajes.push('Valor incorrecto (tiene que ser mayor a 0)');
    }

    if (mensajes.length > 0) {
        e.preventDefault();
        error.innerText = mensajes.join(', '); //Si el arrar tiene longitud, va a mosrar cada mensaje
    }
    else {
        e.preventDefault();
        error.innerText = "";
...
function calculo() { //Funcion calculo
    ...
    document.getElementById('resultado').innerText = `Su costo es $ ${resultado}.`;
    //alert(resultado);
}
